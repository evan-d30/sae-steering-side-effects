#!/usr/bin/env python
# Converted from notebooks/phase123/pythia70m_phase123.ipynb
...
OUT_DIR = Path("/content/pythia70m_phase123_outputs")
...
# Cell 5
################################################################################
# Cell 4 — load Pythia-70M model and residual-stream SAEs

# This updated version uses the actual SAELens release names found in your environment:
#   release = pythia-70m-deduped-res-sm
#   primary = blocks.4.hook_resid_post
#   downstream = blocks.5.hook_resid_post
...
def infer_hook_name(sae_id, model, cfg=None, default_hook=None):
    """Infer the TransformerLens hook name for a SAELens SAE."""
...
W_U = model.W_U.detach().float().to(device)
VOCAB = W_U.shape[-1]
DIRECT_TOPK = [10, 50, 100, 500]

direct_rows = []

for feat in tqdm(selected_features, desc="direct logit footprint"):
    feat = int(feat)
    d = W_DEC_FLOAT[feat].to(device)
    direct_logits = d @ W_U
    abs_logits = direct_logits.abs()
    total_abs = abs_logits.sum().clamp(min=1e-12)

    prob = abs_logits / total_abs
    entropy = -(prob.clamp(min=1e-12) * torch.log(prob.clamp(min=1e-12))).sum()
    ...
    row = {
        ...
        "direct_logit_entropy": float(entropy.item()),
        "direct_logit_effective_vocab": float(effective_vocab.item()),
    }
...
# Cell 28
################################################################################
# Cell 27 — Pythia-70M final verdict / summary
...
summary = {
    ...
    "sae_release_actual": SAE_RELEASE,
    "sae_id_actual": SAE_ID,
    "downstream_sae_id_actual": DOWNSTREAM_SAE_ID,
...
print("=" * 80)
print("PYTHIA-70M PHASE 1/2/3 VERDICT")
print("=" * 80)
...
if (
    ...
    print("VERDICT: PYTHIA SIGNAL SURVIVED.")
    print("Next: run Pythia Phase 4 if the signal is strong.")
elif (
    best_primary_corr is not None
    and abs(best_primary_corr["spearman_r"]) >= 0.20
    and best_full_no_mag_minus_freq >= 0.00
):
    print("VERDICT: PYTHIA PROMISING BUT BORDERLINE.")
    print("Next: inspect targets/predictors before Phase 4.")
else:
    print("VERDICT: WEAK PYTHIA RESULT AFTER ROBUST CONTROLS.")
    print("Next: do not move to Pythia Phase 4 yet. Revisit labels or steering design.")
...
SAVE_TO_DRIVE = True
DRIVE_OUT = "/content/drive/MyDrive/SAE Prediction/pythia70m_phase123_outputs"
