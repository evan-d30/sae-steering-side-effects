#!/usr/bin/env python
# Converted from notebooks/phase123/gpt2_small_phase123.ipynb
...
OUT_DIR = Path("/content/sae_prediction_robust_outputs")
...
try:
    ...
    HOOK_NAME = cfg_dict.get("hook_name", SAE_ID)
...
if USE_DOWNSTREAM_SAE:
    try:
        ...
        downstream_sae.eval()
        try:
            DOWNSTREAM_HOOK_NAME = downstream_sae.cfg.hook_name
        except Exception:
            DOWNSTREAM_HOOK_NAME = downstream_cfg_dict.get("hook_name", DOWNSTREAM_SAE_ID)
...
print("=" * 80)
print("ROBUST PHASE 1/2/3 VERDICT")
print("=" * 80)
...
if (
    ...
    print("VERDICT: ROBUST SIGNAL SURVIVED.")
    print("Next: run a smaller Gemma Scope replication, then Phase 4 screening demo.")
elif (
    best_primary_corr is not None
    and abs(best_primary_corr["spearman_r"]) >= 0.20
    and best_full_no_mag_minus_freq >= 0.00
):
    print("VERDICT: PROMISING BUT BORDERLINE.")
    print("Next: inspect targets/predictors, maybe increase contexts/features before Gemma.")
else:
    print("VERDICT: WEAK AFTER ROBUST CONTROLS.")
...
SAVE_TO_DRIVE = False
DRIVE_OUT = "/content/drive/MyDrive/SAE Prediction/robust_phase123_outputs"
